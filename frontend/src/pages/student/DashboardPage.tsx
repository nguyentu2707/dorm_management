...
export function StudentDashboardPage() {
  ...
  useEffect(() => {
    Promise.all([profileApi.get(), roomChangeApi.mine()])
      .then(([profileData, requests]) => {
      ...
      <div className="mt-6 grid gap-5 xl:grid-cols-3">
        ...
        <InfoPanel title="Thông tin phòng">
          {roomContext.contract && roomContext.room ? (
            ...
            <div className="rounded-xl bg-slate-50 p-5 text-center">
              ...
              <p className="mt-3 font-semibold">Bạn chưa được xếp phòng</p>
              <p className="mt-1 text-sm text-slate-500">
                Chọn phòng và giường phù hợp để gửi đăng ký.
              </p>
              <Link className="btn-primary mt-4" to="/student/contracts">
                Đăng ký ở ngay
              </Link>
            </div>
          )}
        ...
        <InfoPanel title="Thao tác nhanh">
          <div className="space-y-2">
            <QuickAction
              icon={roomContext.contract ? RefreshCw : FilePenLine}
              label={
                roomContext.contract ? "Yêu cầu chuyển phòng" : "Đăng ký ở"
              }
              to={
                roomContext.contract
                  ? "/student/room-change-requests"
                  : "/student/contracts"
              }
            />
            <QuickAction icon={Wrench} label="Báo hỏng thiết bị" disabled />
            <QuickAction
              icon={MessageSquare}
              label="Nhắn tin với BQL"
              disabled
            />
            <QuickAction
              icon={UserRound}
